...
### Для начала работы с удаленными репозиториями проведем регистрацию нашего аккаунта на сайте ` GitHub.com `.  Вводим свое имя, адрес электоронной почты и пароль. После подтверждения электронной почты, можно зайти  в свой профиль и создать новый репозиторий введя его название, можно добавиь его описание.
***Пример***
![Скриншот](сайтgithub.png).
 ### Получив код нашего удаленного репозитория, можно привязать его к нашему лекальному репозиторию. Для этого окткроем на своем компьютере  в терминале `Visual Studio Code` новую папку <Ваше имя новая папка> и введя команду `git clone <код нашего удаленного репозитория>` мы создадим копию оригинального проекта со всей его истрией локально. Для синхронизации текущей ветки с удаленным репозиторием используются команды ` git pull` и `git push `. Команда ` git pull` забирает изменения с удаленного репозитория и проводит слияние с активной веткой. После проведения работы в активной ветке локального репозитория для обновления удаленного репозитория используется команда `git push `. Она отправляет изменения в удаленную ветку, созданную при клонировании по умолчанию.
