...
***Пример***
![Скриншот]().
 ### Получив код нашего удаленного репозитория, можно привязать его к нашему лекальному репозиторию. Для этого окткроем на своем компьютере  в терминале `Visual Studio Code` новую папку <Ваше имя новая папка> и введя команду Sqq
